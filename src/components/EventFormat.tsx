  ...
  return (
    <section className="py-32 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      <div className="mb-20 text-center">
        <h2 className="text-4xl sm:text-5xl font-bold tracking-tight mb-6 text-g-dark inline-block border-b-4 border-g-red pb-2">The 8-Hour Sprint</h2>
        <p className="text-xl text-g-dark font-medium mt-4">Zero fluff. Pure execution.</p>
      </div>

      <div className="relative">
        {/* Continuous bold line */}
        <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-2 bg-g-dark -translate-x-1/2" />
        ...
        <div className="space-y-12">
          {timeline.map((item, i) => (
            ...
            >
              ...
              <div className={`absolute left-6 md:left-1/2 w-8 h-8 rounded-full border-4 border-g-dark ${item.color} -translate-x-1/2 mt-3 md:mt-0 z-10 shadow-brutal-sm`} />

              <div className={`flex-1 pl-16 md:pl-0 ${i % 2 === 0 ? "md:text-left" : "md:text-right"}`}>
                <motion.div 
                  whileHover={{ scale: 1.02, x: i % 2 === 0 ? -5 : 5 }}
                  className="bg-white p-8 rounded-2xl border-4 border-g-dark shadow-brutal inline-block w-full md:w-auto"
                >
                  <span className={`inline-block px-4 py-1.5 rounded-full text-white font-mono font-bold text-sm mb-4 border-2 border-g-dark ${item.color}`}>
                    T+{item.time}
                  </span>
